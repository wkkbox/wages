				...
				<div class="info-table">
					<table class="table" id="order-title">
						...
						<tbody class="table-bordered">
							<tr style="background-color: #F1F1F1;">
								<td colspan="${list.size()+1 }">
									<span class="operation">
										<a href="javascript:void(0);" data-toggle="modal">主工资单详情</a>
									</span>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<c:forEach items="${list }" var="wq" varStatus="i">
									<c:if test="${i.index>12 && i.index<=25 }">
										<c:choose>
											<c:when test="${wq.header=='实发合计'}">
												<td><font color="red">${wq.header }</font></td>
											</c:when>
							...
							<tr>
								<c:forEach items="${list }" var="wq" varStatus="i">
									<c:if test="${i.index>25 && i.index<=38}">
										<c:choose>
											<c:when test="${wq.header=='实发合计'}">
												<td><font color="red">${wq.header }</font></td>
											</c:when>
							...
							<tr>
								<c:forEach items="${list }" var="wq" varStatus="i">
									<c:if test="${i.index>38 && i.index<=51}">
										<c:choose>
											<c:when test="${wq.header=='实发合计'}">
												<td><font color="red">${wq.header }</font></td>
											</c:when>
