...
import com.dylan.core.util.ReMailUtil;
...
public class SendReMailQuartz {
    ...
    public void sendEmail() throws Exception {
        ...
        List<E> selectWagesDetailMail = reParentService.selectWagesDetailMail(formMap);// list中是parentId和身份证号
            ...
            if (size > 250) {
                ...
                for (int i = 0; i < 250; i++) {
                    try {
                        ...
                        // "......e.getStr(managerName)=" +
                        // e.getStr("managerName"));
                        String idCardNO = e.getStr("managerName");// 身份证号码
                        if (idCardNO.trim().length() == 18) {
                            formMap1.put("accountName", e.getStr("managerName"));// 身份证
                            E user = managerService.selectOne(formMap1);// 查到身份证对应的人
                            formMap2.put("id", e.getStr("parentId"));
                            List<E> selectWagesParent = reParentService.selectWagesDetail(formMap2);// 得到工资单详情，header，headerValue，salt
                            ...
                            if (!StringUtil.isEmpty(user.getStr("mail"))) {// 填写了邮箱
                                ...
                                reParentService.editState(formMap3);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
